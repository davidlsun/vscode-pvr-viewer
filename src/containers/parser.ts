...
export default class PVRParser {
    ...
    public constructor(data: Uint8Array) {
        ...
        this.textureInfo = [];
        if (this.depth === 1) {
            this.textureInfo.push({ Name: "Dimensions", Value: `${this.width} x ${this.height}` });
        } else {
            this.textureInfo.push({ Name: "Dimensions", Value: `${this.width} x ${this.height} x ${this.depth}` });
        }
        if (this.pixelFormat === pvr.PixelFormat.NumCompressedPFs) {
            this.textureInfo.push({ Name: "Pixel Format", Value: this.eightcc });
        } else {
            this.textureInfo.push({ Name: "Pixel Format", Value: pvr.PixelFormat[this.pixelFormat].toString() });
        }
        this.textureInfo.push({ Name: "Channel Type", Value: pvr.VariableType[this.channelType].toString() });
        this.textureInfo.push({ Name: "Color Space", Value: pvr.ColorSpace[this.colorSpace].toString() });
        this.textureInfo.push({ Name: "Mip Levels", Value: `${this.mipMapCount}` });
        this.textureInfo.push({ Name: "Faces", Value: `${this.numFaces}` });
        this.textureInfo.push({ Name: "Array Surfaces", Value: `${this.numSurfaces}` });
        ...
        for (let pos = 0; pos < metaDataSize; ) {
            ...
            switch (semantic) {
                ...
                case pvr.MetaData.TextureOrientation:
                    ...
                    this.textureInfo.push({ Name: "Orientation", Value: `${this.flipX ? '-' : '+'}X ${this.flipY ? '-' : '+'}Y ${this.flipZ ? '-' : '+'}Z`});
                    break;
                case pvr.MetaData.BorderData:
                    ...
                    this.textureInfo.push({ Name: "Border Data", Value: `${bx}, ${by}, ${bz}`});
                    break;
                ...
                case pvr.MetaData.MaxRange:
                    ...
                    this.textureInfo.push({ Name: "Max Range", Value: `${this.maxRange}`});
                    break;
